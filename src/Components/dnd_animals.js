...
const DraggableThing = (props) => {
  ...
  return <div className="draggableThing" id={props.id} draggable="true" onDragStart={dragStart} />
}
...
const DroppableArea = (props) => {
  ...
  const allowDrop = (e) => {
    // The default action of onDragOver is to cancel the drop operation /so we need to prevent that
    e.preventDefault()
  }
  // https://developer.mozilla.org/en-US/docs/Web/API/Document/dragenter_event
  const dragEnter = (e) => {
    // Drag Enter is used to highlight the drop area
  ...
  return (
    <div className="droppableArea" id={props.id} onDrop={drop} onDragOver={allowDrop} onDragEnter={dragEnter} onDragLeave={dragLeave}>
      <p>picked animals 🤲</p>
    </div>
  )
}
